...
先確認真實程式碼與部署設定，再把它整理成能定位、能理解、能對照、能查證的主題式文章。網站以 API、CI/CD、Cloud Run、效能與實際案例分類，不使用連續章節編號，也不設計 Lab 實作課程。
...
7. 描述現行專案時要附設定依據，讓讀者能區分通用觀念與專案事實。來源可直接併入「本專案採用方式」或案例表格；只有設定漂移、時間差或查證過程本身是主題時，才獨立建立「實際設定查證」與查證日期，不要用兩張表重複相同結論。
8. 文章以架構、技術名詞、元件責任、資料流與設定解讀為主，不加入 Lab、環境需求、操作步驟或打勾式驗證結果。若操作風險本身是主題，例如正式壓測或部署變更，改以「風險邊界」「可能影響」「恢復觀察」解釋，不寫成讓讀者照做的練習。
...
- 主題若同時有份量足夠的通用原理與 `ai-asst-km` 實際架構決策，可拆成講義（通用技術說明，不綁專案細節）與案例子章節（`ai-asst-km` 現行架構）兩篇，共用同一份文章骨架；講義著重基礎觀念，案例著重實際案例與本專案採用方式，分類導讀頁的閱讀順序要把兩篇配對呈現。份量不足以拆分的主題仍維持單篇混寫。
- Flask + Gunicorn 是現行 API 技術；不要誤寫成 FastAPI。
- API 與部署文章應解釋「是什麼」「位於哪一層」「為什麼需要」「如何與其他元件互動」，再用實際設定查證現況。
- 不揭露 Project ID、完整 Service URL、Secret 值、Token、帳號或內部資料。
- 無法由程式碼或唯讀查詢確認的內容，明確標成推測、待確認或一般示例。
- 不建立空白文章；分類導讀只能連結已存在且具有實質內容的文章。
- Markdown 檔名、H1 與導覽都不加 `第 N 章`、`01`、`02` 等序號。檔名使用能表達主題的 snake_case，例如 `cloud_run_core_concepts.md`；內部連結使用文章名稱，不使用「上一章／下一章」。
...
- 專題文章包含主題範圍、基礎觀念、實際案例與設定依據；分類導讀包含技術地圖、術語與閱讀路徑。
- 章節目標、名詞解釋、架構圖、實際案例、查證來源與小結互相對應。
- 技術敘述與實際 Repository 或官方文件一致。
- 文章沒有 Lab、環境需求、操作步驟或打勾式驗證結果；具風險的正式操作只解釋邊界與影響。
- 導覽與內部連結有效。
- `docs/` 根目錄沒有以數字序號開頭的文章檔名。
