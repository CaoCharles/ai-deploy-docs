# 雲端架構與 API 筆記格式契約

新增或重整文章時使用這份契約。網站採主題分類，不使用連續章節編號；區塊深度依主題調整，不為了湊格式加入無關內容。

## 固定骨架

```markdown
---
authors:
  - name: Charles Cao
tags:
  - Topic
---

# 主題名稱

## 學習目標

- [ ] 可觀察、可驗證的學習成果

## 這篇筆記涵蓋的範圍

使用一段精確文字或一張適合的圖標示這篇筆記涵蓋的元件與上下游。正式 C4／Deployment 架構使用 draw.io PNG 並提供 source；小型流程才使用 Mermaid。

## 前置知識

## 核心問題或情境

## 基礎觀念

!!! info "基礎觀念"
    說明適用於一般系統的技術概念。

## 實際專案案例

!!! example "實際案例"
    說明目前程式碼或雲端設定採取的方式；可使用 ai-asst-km，但不要把它當成整個網站品牌。

## 本專案採用方式

| 項目 | 現行做法 | 設定依據 |
|---|---|---|
| 啟動方式 | Flask + Gunicorn | Dockerfile／啟動命令 |

不要貼 Secret 值、完整 Service URL、Token、Project ID 或內部帳號。若尚未確認，將現行結論標示為「待確認」，不要猜測。

若文章主題涉及設定漂移、雲端現況與 Repository 差異，才改用「實際設定查證」表格並記錄查證日期。一般技術解說不要把案例與查證拆成兩張內容重複的表格。

## 架構與設定解讀

使用圖、表格或短段落解釋元件所在層次、責任、上下游與現行設定。這個區塊不是操作 Lab，不要求讀者準備 Repository、登入雲端或執行命令。

## 常見問題

## 小結

## 延伸閱讀
```

`date` 與 `updated` 不寫入 frontmatter。網站使用 Git history 自動產生建立日期與更新日期，避免文章內容與日期出現兩套來源；GitHub Pages checkout 必須保留完整歷史。

## 分類導讀頁

分類導讀頁不必套用完整專題骨架，但至少要包含：

- 一個不帶編號的 H1。
- 學習目標。
- 技術地圖或核心術語對照。
- 只連結已完成文章的建議閱讀順序。
- 實際案例或設定對照（適用時）。
- 官方延伸閱讀。

## 章節選擇原則

- 架構總覽：先分 Runtime 與 Deployment，再介紹元件責任與資料流；全貌圖本身可以同時作為章節定位。
- HTTP/API：從 Request、Response、Method、Path、JSON、Status Code 開始，再對照 Flask route。
- Gunicorn：先說明 Flask 開發伺服器的限制，再解釋 WSGI、worker、timeout 與 Cloud Run 的關係。
- Docker：從 Dockerfile 每一層與 Image/Container 差異切入，再連到 Artifact Registry。
- GitHub Actions：拆解 trigger、job、step、WIF、build、push、deploy，不直接貼整份 workflow 取代說明。
- Cloud Run：說明 Service、Revision、Instance、traffic、scale to zero、cold start、環境變數與 Secret。
- 維運：涵蓋 logs、health check、rollback、權限與設定漂移，但任何變更正式環境的步驟都要標出風險。

## 操作內容界線

- 網站定位是架構與技術知識庫，不設計 Lab、環境準備、逐步操作或驗證打勾清單。
- 指令只在它本身能幫助解釋啟動命令、設定語法或元件關係時作為短例子，不要求讀者執行。
- 正式壓測、部署、流量或 Secret 變更等高風險操作，只說明風險邊界、可能影響與恢復觀察；不把說明視為操作授權。

## 風格界線

- 以短段落、精確表格和必要圖表為主，不把每段文字都包成 Admonition。
- 「基礎觀念」只說通用原理；「本專案採用方式」只說具有設定依據的現況。
- Markdown 檔名、H1、導覽與內部連結不用 `01`、`02`、`第 N 章` 或「上一章／下一章」；檔名使用主題式 snake_case。
- 查證來源優先寫 Repository 內的相對位置或服務種類，不把本機絕對路徑寫進公開文章。
- 延伸閱讀優先連結官方文件；第三方文章只在能補足實務觀點時加入。
- draw.io 圖後必須說明圖表類型、閱讀順序與邊界；不要只放一張沒有解說的圖片。
